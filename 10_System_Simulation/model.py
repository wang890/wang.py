...
class RCsys(Component):
    ...
    def __init__(self):
        super().__init__('sys')
        # 实例化元件
        self.resistor = Resistor('resistor', R=1.0)
        self.capacitor = Capacitor('capacitor', C=1.0)
        self.source = ConstantVoltage('source', V=1.0)
...
# 上述相当于库文件，下面是对上述的测试，一般写的代码文件都要这样测试
